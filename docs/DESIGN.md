...
A contributor proposed a rounded bordered card for the expanded detail plus a
red capsule "Quit" button. Declined, because it imports the System Settings
*window* idiom (cards, pill buttons) into a menu bar panel. That is the wrong
layer: settings-window chrome inside a content-layer popover. The band-not-card
and plain-Quit choices above are the deliberate alternative.
