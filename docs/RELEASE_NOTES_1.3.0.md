...
- Multiple external displays no longer cross-talk: adjusting one monitor's
  ...
  correct display (identity first, proximity fallback). (#14, #13) `(pending: #13 merge + verification)`
...
- DDC brightness writes paced to ~20/sec to prevent flicker on fast drags. (#13) `(pending: #13 merge)`

## Dev / build

- `dev.sh` signs with a stable identity so the Accessibility grant survives
  rebuilds.
- Added a `Makefile` wrapping the existing build scripts. (#13) `(pending: #13 merge)`

## Contributors

- @shaw-baobao: color profile subtitle sync (#11)
- @caicaiks: multi-display DDC matching and reply-frame hardening (#13) `(pending merge)`
